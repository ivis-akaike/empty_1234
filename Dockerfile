...
RUN conda update --yes setuptools
...
RUN pip install --no-cache-dir git+https://github.com/NII-cloud-operation/Jupyter-LC_run_through
RUN pip install --no-cache-dir git+https://github.com/NII-cloud-operation/Jupyter-multi_outputs
RUN pip install --no-cache-dir datalad==0.15.4
RUN pip install --no-cache-dir lxml==4.7.1
RUN pip install --no-cache-dir blockdiag==3.0.0
RUN pip install --no-cache-dir -U nbformat==5.2.0
RUN pip install --no-cache-dir papermill==2.3.3
RUN pip install --no-cache-dir black==21.12b0
